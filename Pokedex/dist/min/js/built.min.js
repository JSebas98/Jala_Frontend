/*!  2022-05-23 */
const popoverTriggerList=document.querySelectorAll('[data-bs-toggle="popover"]'),popoverList=[...popoverTriggerList].map(e=>new bootstrap.Popover(e)),offset=0,template=(fetch(`https://pokeapi.co/api/v2/pokemon/?offset=${offset}&limit=25`).then(e=>e.json()).then(e=>{let o=e.results;o.forEach((e,o)=>{let a=document.getElementById("pokedex"),t=document.createElement("poke-card");var p=pokemonColorMap[o+offset+1];t.setAttribute("p-image",getPokemonImageUri(o+offset+1)),t.setAttribute("p-name",e.name),t.setAttribute("color",p),a.appendChild(t)}),customElements.define("poke-card",PokemonCard)}).catch(e=>{console.log(e)}),document.createElement("template"));template.innerHTML=`
<style>
    .pokemon-card {
        display: flex;
